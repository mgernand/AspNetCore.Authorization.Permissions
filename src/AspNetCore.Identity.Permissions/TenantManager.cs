namespace MadEyeMatt.AspNetCore.Identity.Permissions
{
        ...
        where TTenant : class, ITenant
    {
        ...
        public virtual IQueryable<TTenant> Tenants
        {
            get
            {
                IQueryableTenantStore<TTenant> queryableStore = Store as IQueryableTenantStore<TTenant>;
                if (queryableStore == null)
                {
                    throw new NotSupportedException("The permission store is not an IQueryableTenantStore." /*Resources.StoreNotIQueryableRoleStore*/);
                }
        ...
        public virtual async Task<IdentityResult> CreateAsync(TTenant tenant)
        {
            ...
            IdentityResult result = await ValidateTenantAsync(tenant);
            if (!result.Succeeded)
            {
                return result;
            }
            ...
            result = await Store.CreateAsync(tenant, CancellationToken);
            return result;
        }
        ...
        public virtual async Task<bool> TenantExistsAsync(string tenantName)
        {
            ...
            return await FindByNameAsync(tenantName) != null;
        }
        ...
        public Task<TTenant> FindByNameAsync(string tenantName)
        {
            ...
            return Store.FindByNameAsync(NormalizeName(tenantName), CancellationToken);
        }
        ...
        {
            string name = await GetTenantIdAsync(tenant);
            await Store.SetNormalizedTenantNameAsync(tenant, NormalizeName(name), CancellationToken);
        }
        ...
        {
            ThrowIfDisposed();
            return Store.GetTenantIdAsync(tenant, CancellationToken);
        }
        ...
        {
            ThrowIfDisposed();
            return Store.GetTenantNameAsync(tenant, CancellationToken);
        }
        ...
        {
            ThrowIfDisposed();
            return Store.GetTenantDisplayNameAsync(tenant, CancellationToken);
        }
        ...
        ///     of the operation.
        /// </returns>
        public virtual async Task<IdentityResult> SetTenantNameAsync(TTenant tenant, string name)
        {
            ThrowIfDisposed();

            await Store.SetTenantNameAsync(tenant, name, CancellationToken);
            await UpdateNormalizedTenantNameAsync(tenant);
        ...
        public virtual string GetTenantName(ClaimsPrincipal principal)
        {
            if (principal == null)
            {
            ...
            return principal.FindFirstValue(Authorization.Permissions.Abstractions.PermissionClaimTypes.TenantNameClaimType);
        }
        ...
        protected virtual async Task<IdentityResult> ValidateTenantAsync(TTenant tenant)
        {
            ...
            if (errors.Count > 0)
            {
                Logger.LogWarning(
                    ...
                    "Permission {permissionId} validation failed: {errors}.", await GetTenantIdAsync(tenant), string.Join(";", errors.Select(e => e.Code)));
        ...
        {
            ThrowIfDisposed();
            ITenantRoleStore<TTenant> store = GetTenantRoleStore();
            if (tenant == null)
            {
                throw new ArgumentNullException(nameof(tenant));
            }

            return await store.GetRolesAsync(tenant, CancellationToken);
        }

        private ITenantRoleStore<TTenant> GetTenantRoleStore()
        {
            if (Store is not ITenantRoleStore<TTenant> cast)
            {
