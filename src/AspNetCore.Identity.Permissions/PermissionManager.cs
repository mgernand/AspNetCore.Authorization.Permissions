namespace MadEyeMatt.AspNetCore.Identity.Permissions
{
        ...
        where TPermission : class, IPermission
    {
        ...
        public virtual IQueryable<TPermission> Permissions
        {
            get
            {
                IQueryablePermissionStore<TPermission> queryableStore = Store as IQueryablePermissionStore<TPermission>;
        ...
        public virtual async Task<IdentityResult> CreateAsync(TPermission permission)
        {
            ...
            IdentityResult result = await ValidatePermissionAsync(permission);
            if (!result.Succeeded)
            {
                return result;
            }
        ...
        public virtual async Task<bool> PermissionExistsAsync(string permissionName)
        {
            ...
            return await FindByNameAsync(permissionName) != null;
        }
        ...
        {
            string name = await GetPermissionNameAsync(permission);
            await Store.SetNormalizedPermissionNameAsync(permission, NormalizeName(name), CancellationToken);
        }
        ...
        public Task<TPermission> FindByNameAsync(string permissionName)
        {
            ...
            return Store.FindByNameAsync(NormalizeName(permissionName), CancellationToken);
        }
        ...
        protected virtual async Task<IdentityResult> ValidatePermissionAsync(TPermission permission)
        {
            ...
            if (errors.Count > 0)
            {
                Logger.LogWarning(
                    ...
                    "Permission {permissionId} validation failed: {errors}.", await GetPermissionIdAsync(permission), string.Join(";", errors.Select(e => e.Code)));
        ...
        {
            ThrowIfDisposed();
            IRolePermissionStore<TPermission> store = GetRolePermissionStore();
            if (roleName == null)
            {
                throw new ArgumentNullException(nameof(roleName));
            }

            return store.GetPermissionsInRoleAsync(NormalizeName(roleName), CancellationToken);
        }

        private IRolePermissionStore<TPermission> GetRolePermissionStore()
        {
            IRolePermissionStore<TPermission> cast = Store as IRolePermissionStore<TPermission>;
            if (cast == null)
            {
